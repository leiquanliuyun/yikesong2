...
class  PactController extends Controller
{
    ...
    public function pact_add()
    {
        ...
        if(request()->isPost()) {
            ...
            foreach ($mess as $key=>$val){
                $mess[$key]=strip_tags(htmlspecialchars_decode($val));
            }
            ...
            if($result) {
                ...
                $this->success('合同添加成功', url('member/member_detail',['id'=>$param['uid'],'index'=>1]),'',3);
            } else {
                $this->error('操作失败！');
            }
        }
        ...
        $pactoption=model('PactOption');
        $alloptions=$pactoption->select();//var_dump($alloptions);exit;//加上filed下面json_encode就会报错
        $stringoptions=json_encode($alloptions);//var_dump($stringoptions);exit;
        //获取订单信息
        $order=model('Order');
        ...
        $this->assign('temp', $temp);//默认显示第一个合同模板
        $this->assign('id', $id);
    ...
    public function changeTemp(){
        if (request()->isAjax()){
            ...
            $pactoption=model('PactOption');
            $alloptions=$pactoption->field('option_id,name')->order('option_id')->select();
            $data['options']=$options;
            $data['temp_id']=$param['id'];
    ...
    public function pact_edit()
    {
        ...
        if(request()->isPost()) {
            ...
            $validate = validate('Pact');
            //表单提交数据
            $param = input('post.'); //echo '<pre>' ;var_dump($param);
            //拼合同内容
            $mess=$param;
            unset($mess['ordernum']);
            unset($mess['name']);
            ...
            foreach ($mess as $key=>$val){
                $mess[$key]=strip_tags(htmlspecialchars_decode($val));
            } //echo '<pre>' ;var_dump($mess);
            $param['detail']=json_encode($mess);//halt($param['detail']);
            //验证 表单数据
            if(!$validate->check($param)){
                $this->error($validate->getError());
            }
            ...
            if($result) {
                ...
                //$this->success('操作成功', 'pact/pact','',1);
                //$this->redirect( 'member/member_detail',['id'=>$param['uid']],5,'操作成功');
                $this->success('合同修改修改成功', url('member/member_detail',['id'=>$param['uid'],'index'=>1]),'',3);
            } else {
                $this->error('操作失败！');
            }
        }
        ...
        $option = empty($option) ? 'add':$option;
        //获取所有合同项
        $id =  input('param.order_id');//订单id,员工端全部传合同id
        $pactoption=model('PactOption');
        $alloptions=$pactoption->select();
        $stringoptions=json_encode($alloptions);
        //获取合同信息，对于催缴生成的订单查其合同时用belong_id
        $rs=db('order')->where('id',$id)->field('belong_id')->find();
        $pact=model('Pact');
        $order_id=$rs['belong_id']?$rs['belong_id']:$id;
        $list=$pact->where('order_id',$order_id)->find();
        //合同内容处理
