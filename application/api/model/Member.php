...
/**
 * 会员表 模型  用户头像因为存在默认，微信头像，及上传3种情况，故统一存绝对路径（包括域名）
 */
class Member extends Common
{
   ...
   /* public function getPictureShowAttr($value,$data)
    ...
    public function getIdPhotoShowAttr($value,$data)
    {
        ...
        return $web_url.__PUBLIC__."/uploads/images/".$data['id_photo'];
    }
    //在获取数据的字段值后自动进行处理
     ...
     * @param $user_type integer   用户类型 0消费会员,1商家,2服务员
     * @return User 对象实例
     */
    ...
    public function register($check_sms = true){
        ...
        $data = [
            ...
            'picture'=>config('web_url').'/static/images/mine.png',
            'code'=>uniqid()
        ];
        //上级code
        if ($this->pid){
